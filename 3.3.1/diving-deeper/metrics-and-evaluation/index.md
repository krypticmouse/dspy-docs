...
- [Settings and `context()`](https://dspy.ai/3.3.1/diving-deeper/settings-and-context/index.md) — how `Evaluate`‘s workers inherit a `dspy.context(...)` override around the call.
- [Built-in module variants](https://dspy.ai/3.3.1/diving-deeper/built-in-module-variants/index.md) — `dspy.Parallel` and `Module.batch` use the same `ParallelExecutor` under the hood.
- [Optimizers: choosing one](https://dspy.ai/3.3.1/diving-deeper/choosing-an-optimizer/index.md) — every optimizer compiles against a metric defined here; the `Prediction(score, feedback)` shape GEPA expects is documented above.
