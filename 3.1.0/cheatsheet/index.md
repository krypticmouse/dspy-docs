...
Detailed documentation on BootstrapFewShotWithRandomSearch can be found [here](../api/optimizers/BootstrapFewShot/).
...
Detailed documentation on COPRO can be found [here](../api/optimizers/COPRO/).
...
Note: detailed documentation can be found [here](../api/optimizers/MIPROv2/). `MIPROv2` is the latest extension of `MIPRO` which includes updates such as (1) improvements to instruction proposal and (2) more efficient search with minibatching.

#### Optimizing with MIPROv2

This shows how to perform an easy out-of-the box run with `auto=light`, which configures many hyperparameters for you and performs a light optimization run. You can alternatively set `auto=medium` or `auto=heavy` to perform longer optimization runs. The more detailed `MIPROv2` documentation [here](../api/optimizers/MIPROv2/) also provides more information about how to set hyperparameters by hand.
