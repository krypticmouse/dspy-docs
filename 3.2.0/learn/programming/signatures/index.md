...
1. Sentiment Classification: `"sentence -> sentiment: bool"`, e.g. `True` if positive
1. Summarization: `"document -> summary"`
...
1. Multiple-Choice Question Answering with Reasoning: `"question, choices: list[str] -> reasoning: str, selection: int"`
...
1. Supply hints on the nature of an input field, expressed as a `desc` keyword argument for `dspy.InputField`.
1. Supply constraints on an output field, expressed as a `desc` keyword argument for `dspy.OutputField`.
...
1. **Typing module types** like `list[str]`, `dict[str, int]`, `Optional[float]`. `Union[str, int]`
1. **Custom types** defined in your code
1. **Dot notation** for nested types with proper configuration
1. **Special data types** like `dspy.Image, dspy.History`
...
DSPy automatically validates that the values you pass to input fields match the types specified in your signature. This works for both inline and class-based signatures. When there's a type mismatch, DSPy will log a warning.
...
You should compose multiple signatures into bigger [DSPy modules](https://dspy.ai/3.2.0/learn/programming/modules/index.md) and [compile these modules into optimized prompts](https://dspy.ai/3.2.0/learn/optimization/optimizers/index.md) and finetunes.
