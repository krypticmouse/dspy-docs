...
`ProgramOfThought` is deprecated and will be removed in DSPy 3.5. [`RLM`](https://dspy.ai/3.4.0b1/api/modules/RLM/index.md) is the preferred replacement.

## `dspy.ProgramOfThought(signature: str | type[Signature], max_iters: int = 3, interpreter_factory: Callable[[], CodeInterpreter] = PythonInterpreter)`

Bases: `Module`

.. deprecated:: 3.4 ProgramOfThought is deprecated. RLM is the preferred replacement.

A DSPy module that runs Python programs to solve a problem. This module requires Deno. Install DSPy’s managed runtime with `pip install "dspy[deno]"`.
...
| `max_iters`           | `int`                           | The maximum number of iterations to retry code generation and execution.                                                                                             | `3`                          |
...
#### `forward(interpreter: CodeInterpreter | None = None, /, **kwargs)`
...
| Name          | Type              | Description                                         | Default                                                                               |
| ------------- | ----------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `interpreter` | \`CodeInterpreter | None\`                                              | Optional caller-owned interpreter, passed positionally. The caller must shut it down. |
