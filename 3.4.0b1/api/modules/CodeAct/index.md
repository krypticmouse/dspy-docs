...
`CodeAct` is deprecated and will be removed in DSPy 3.5. [`RLM`](https://dspy.ai/3.4.0b1/api/modules/RLM/index.md) is the preferred replacement.

## `dspy.CodeAct(signature: str | type[Signature], tools: list[Callable], max_iters: int = 5, interpreter_factory: Callable[[], CodeInterpreter] = PythonInterpreter)`

Bases: `ReAct`, `ProgramOfThought`

.. deprecated:: 3.4 CodeAct is deprecated. RLM is the preferred replacement.
...
| --------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| `signature`           | `Union[str, Type[Signature]]`   | The signature of the module.                                                                                                                                         | *required*          |
| `tools`               | `list[Callable]`                | The tool callables to be used. CodeAct only accepts functions and not callable objects.                                                                              | *required*          |
| `max_iters`           | `int`                           | The maximum number of iterations to generate the answer.                                                                                                             | `5`                 |
| `interpreter_factory` | `Callable[[], CodeInterpreter]` | Zero-argument callable that creates an interpreter for each forward pass. The callable may be invoked concurrently, and DSPy shuts down each interpreter it returns. | `PythonInterpreter` |
...
1. Generates Python code snippets that use these tools
1. Executes the code using a Python sandbox
1. Collects the output and determines if the task is complete
1. Answer the original question based on the collected information
