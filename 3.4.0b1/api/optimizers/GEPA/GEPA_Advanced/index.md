...
`code_proposer` lets you supply your own function for rewriting the source code of a [`dspy.Flex`](https://dspy.ai/3.4.0b1/diving-deeper/flex/index.md) submodule during GEPA optimization. GEPA calls it each reflection round with the current source and the examples it ran on, and it returns a revised `dspy.Module` class.
