...
To turn our Signature into a callable function, we use `dspy.Predict`. Predict is a kind of [DSPy `Module`](https://dspy.ai/3.4.0b1/diving-deeper/modules/index.md). If Signatures specify *what* we want, Modules define *how* we aim to achieve it. They implement a call-time strategy, manage the control flow, tools, and more.
...
1. A default instruction string is generated for the `Signature` instance, in this case: “Given the fields `subject`, produce the fields `haiku`.”
1. A `Predict` module is instantiated with this `Signature`.
...
1. An [`Adapter`](https://dspy.ai/3.4.0b1/diving-deeper/adapters/index.md) is used to render the `Signature` and its inputs into messages the `LM` can consume. By default, this is the `ChatAdapter`, but there are JSON, XML, and other variants to format your messages suitable for a given `LM`.
1. The `ChatAdapter` builds the prompt, which includes the `Signature` instructions, the field schema describing the inputs and outputs, formatting instructions, and the provided input (in this case, “computer science”).
1. The messages are sent to the `LM`. Caching is enabled by default so identical calls return cached responses.
1. A response is returned, which the `ChatAdapter` parses to extract the output fields.
1. The `Predict` module returns a `Prediction` object with accessible output fields. Calling `result.haiku` returns the generated haiku.
1. The call is recorded in `LM` history, which can be inspected later with `dspy.inspect_history()`.
...
**Next:** [Expanding signatures →](https://dspy.ai/3.4.0b1/getting-started/expanding-signatures/index.md)
