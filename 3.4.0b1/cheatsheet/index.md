...
DSPy caches LM calls. Provide a unique `rollout_id` and set a non-zero `temperature` (e.g., 1.0) to bypass an existing cache entry while still caching the new result:
...
Detailed documentation on BootstrapFewShotWithRandomSearch can be found [here](https://dspy.ai/3.4.0b1/api/optimizers/BootstrapFewShot/index.md).
...
Detailed documentation on COPRO can be found [here](https://dspy.ai/3.4.0b1/api/optimizers/COPRO/index.md).
...
Note: detailed documentation can be found [here](https://dspy.ai/3.4.0b1/api/optimizers/MIPROv2/index.md). `MIPROv2` is the latest extension of `MIPRO` which includes updates such as (1) improvements to instruction proposal and (2) more efficient search with minibatching.

#### Optimizing with MIPROv2

This shows how to perform an easy out-of-the box run with `auto=light`, which configures many hyperparameters for you and performs a light optimization run. You can alternatively set `auto=medium` or `auto=heavy` to perform longer optimization runs. The more detailed `MIPROv2` documentation [here](https://dspy.ai/3.4.0b1/api/optimizers/MIPROv2/index.md) also provides more information about how to set hyperparameters by hand.
