...
DSPy ships with [several prompt optimizers](https://dspy.ai/current/diving-deeper/choosing-an-optimizer/index.md), but today we’re going to focus on GEPA.
...
1. Examples and their metric results are sent to the reflection LM, which proposes new instructions.
1. GEPA runs the program again with the new instructions, scores again, and keeps the instruction candidates that score best.
...
See [GEPA in depth](https://dspy.ai/current/diving-deeper/gepa-in-depth/index.md) for the full mechanics: Pareto sampling, per-predictor feedback, `auto` budget translation, and the `detailed_results` audit trail.

______________________________________________________________________

**Next:** [Saving and loading →](https://dspy.ai/current/getting-started/saving-and-loading/index.md)
