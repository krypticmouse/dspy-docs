...
`Flex` always runs generated code in a sandbox (`interpreter_factory` defaults to `dspy.PythonInterpreter`), which requires [Deno](https://deno.land/) for its Pyodide WASM sandbox — see the [RLM page](https://dspy.ai/3.3.0/api/modules/RLM/#deno-installation) for installation notes.
