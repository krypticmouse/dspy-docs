...
Use `dspy.context(...)` when you need a different LM, adapter, or flag for one part of your program without changing the process-wide defaults from [`dspy.configure`](https://dspy.ai/3.3.0/api/utils/configure/index.md). The block inherits every current setting, overrides only the keys you pass, and restores the originals when it exits.

`dspy.context(...)` accepts the same settings as [`dspy.configure`](https://dspy.ai/3.3.0/api/utils/configure/#settings).
...
Unlike [`dspy.configure`](https://dspy.ai/3.3.0/api/utils/configure/index.md), you can call `dspy.context(...)` from **any** thread or async task. This makes it the right tool for overrides inside [`dspy.Parallel`](https://dspy.ai/3.3.0/api/modules/Parallel/index.md), `asyncio.gather`, or any concurrent code.

Settings inside a `dspy.context(...)` block do not leak to other threads or tasks.

## See Also

- [`dspy.configure`](https://dspy.ai/3.3.0/api/utils/configure/index.md) — set process-wide defaults.
