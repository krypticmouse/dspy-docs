...
**`dspy.ReAct`** is the canonical tool-using loop and has its own page: [Tools, ReAct, and MCP](https://dspy.ai/3.3.0/diving-deeper/tools-react-and-mcp/index.md). The wrapping machinery there is what `CodeAct` and `RLM` reuse.

## Cross-links

- [Modules: composing your own](https://dspy.ai/3.3.0/diving-deeper/modules/index.md) — every variant here is a `dspy.Module` (except `Parallel` and `majority`), so the composition rules apply.
- [Tools, ReAct, and MCP](https://dspy.ai/3.3.0/diving-deeper/tools-react-and-mcp/index.md) — `CodeAct` and `RLM` use the same tool-wrapping machinery as `ReAct`.
- [RLM: exploring large contexts with code](https://dspy.ai/3.3.0/diving-deeper/rlm/index.md) — the deep dive on the experimental REPL-driven module summarized above.
- [Flex: Optimizable module code](https://dspy.ai/3.3.0/diving-deeper/flex/index.md) — the deep dive on the code-optimizable Flex module summarized above.
- [Settings and `context()`](https://dspy.ai/3.3.0/diving-deeper/settings-and-context/index.md) — how `Parallel` and `Module.batch` snapshot the active overrides into each worker.
